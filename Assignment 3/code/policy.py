...
class BasePolicy:
    ...
    def act(self, observations, return_log_prob = False):
        ...
        #########          END YOUR CODE.          ############
        if return_log_prob:
            log_probs = act_dist.log_prob(sampled_actions)
            return sampled_actions.detach().cpu().numpy(), log_probs.detach().cpu().numpy()
        return sampled_actions.detach().cpu().numpy()
...
class GaussianPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        action_dim = loc.size(-1)
        distribution = ptd.MultivariateNormal(loc=loc, scale_tril=cov.view(-1, action_dim, action_dim))
        return distribution